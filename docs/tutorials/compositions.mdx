...
The composed frame puts the race full-screen with the player in a small overlay in the top-left corner, standard gaming stream layout. Halfway through, the layout switches to a side-by-side grid without interrupting the stream, which is what you will do in Step 6:
