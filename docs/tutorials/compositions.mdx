...
- A Fishjam account. Open the [**Fishjam developer panel**](https://fishjam.io/app) and copy your **Fishjam ID** and **Management Token** from the Dashboard tab, and your **Sandbox API URL** from the Sandbox tab.
- A browser, to watch the result.

The [Composition API](../api/reference#compositions) lives on `https://rtc.fishjam.io` and takes your Management Token. The livestream you will publish to comes from the [Sandbox API](../explanation/sandbox-api-concept), which needs no token at all:
...
The composition needs somewhere to send the composed stream. The [Sandbox API](../explanation/sandbox-api-concept) creates a [livestream](./../explanation/livestreams) and hands back a **streamer token** in one unauthenticated call. Making it public lets viewers watch without a token, which keeps this tutorial short:

```bash
curl "$SANDBOX_URL/room-manager/livestream?roomName=composition-tutorial&public=true"
```
...
:::note
The Sandbox API is for prototyping: it holds your Management Token server-side so a client can get a scoped token without a backend. In production you create livestreams yourself through the [Fishjam Server API](../api/reference#server), with your Management Token. Nothing else in this tutorial changes.
:::
...
Open the [livestreaming demo](https://livestreaming.fishjam.io), switch to watching, and give it your **Fishjam ID**, your **Sandbox API URL**, and the room name you used in Step 3, `composition-tutorial`. It fetches a viewer token for you and plays the stream.

You should see the race running full-screen with the player tucked into the top-left corner.

Because the livestream is public, any WHEP player works too, without a token. The playback URL is the WHEP endpoint with the stream id on the end:
