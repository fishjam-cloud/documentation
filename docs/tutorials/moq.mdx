...
Import once to register the element:

```ts
import "@moq/watch/element";
```

Then drop `<moq-watch>` into your HTML — pass the relay URL (with the JWT token in the query string) and the broadcast path:
...
const connection = new Moq.Connection.Reload({
  url: new Signal(new URL("https://relay.example.com/room")),
