...
export function JoinRoomButton() {
  ...
  const handleJoinRoom = async () => {
    ...
    const cameraDevices = navigator.mediaDevices
      .enumerateDevices()
      .then((devices) =>
        devices.filter((device) => device.kind === "videoinput"),
      );
    const firstCamera = (await cameraDevices)[0];
    if (firstCamera) {
      await selectCamera(firstCamera.deviceId);
    }
...
function VideoCall() {
  ...
  const { selectCamera, cameraStream } = useCamera();
  const { remotePeers } = usePeers();
