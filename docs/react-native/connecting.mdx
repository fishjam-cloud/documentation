...
In order to connect, you need to retrieve **Fishjam URL** and **Peer Token** (token that will authenticate the peer in
your Room).
...
with pre-configured test service called [Room Manager](/room-manager). This is basically service that will create Room, add your app as
...
For production app, you need to implement your own backend service that will provide user with **Peer Token** To do that,
