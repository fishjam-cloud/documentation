...
On Android API level >= 24, you must use a foreground service when screensharing. This will be handled automatically for you. If you'd like to configure service behaviour (like notification content, other permissions). Checkout `useForegroundService` hook.
