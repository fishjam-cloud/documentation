...
- Setting up livestreaming in React Native apps
- Broadcasting video and audio streams
- Receiving and displaying livestreams
...
**Difficulty level:** Intermediate (requires basic React Native knowledge)

## 🎯 Target Audience

**Primary audience:** React Native developers who want to add livestreaming capabilities to their apps.

**Skill level:** You should be familiar with:
...
- **Room of `livestream` type** created on your Fishjam server
- **Appropriate tokens** (peer token for streaming, viewer token for receiving)
- **Camera and microphone permissions** configured

See the [livestreaming overview](/livestreaming) for background information about Fishjam's livestreaming architecture.
...
- **Automatic distribution** to all connected viewers

## 🚀 Quick Start

If you already have some experience with Fishjam and want to jump straight in:

1. **Install the SDK:** `npm install @fishjam-cloud/react-native-client`
2. **Configure permissions** (see Required Permissions section)
3. **Copy the Streamer Example** or **Viewer Example** below
4. **Replace placeholder values** with your actual room details
...
- A [peer token](/glossary#peer-token) for that room
- Camera and microphone permissions configured

### Setup Steps

1. **Join the room** using the [connection guide](/react-native/connecting)
2. **Start streaming** following the [streaming guide](/react-native/start-streaming)
...
- A [viewer token](/livestreaming) for that room
...
:::info
This guide assumes you completed either the [Quick Setup](/react-native/quick-setup) or [Installation](/react-native/installation) guide.
:::
...
- ✅ Verify the streamer successfully joined the room
- ✅ Check that the viewer token is valid and not expired
- ✅ Ensure the room type is `livestream`

**PiP not working:**

- ✅ Confirm PiP support is enabled in `app.json`
- ✅ On Android, ensure the app is in the background
...
- ✅ Confirm your Fishjam server is running

## 📚 Additional Resources

- [Livestreaming Overview](/livestreaming) - Learn about Fishjam's livestreaming architecture
- [Connection Guide](/react-native/connecting) - Detailed connection setup
- [Streaming Guide](/react-native/start-streaming) - Advanced streaming configuration
