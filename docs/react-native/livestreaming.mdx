...
**Target audience:** React Native developers who want to add livestreaming capabilities to their apps.

**Prerequisites:** You need a room of `livestream` type and appropriate tokens. See the [livestreaming overview](/livestreaming) for background information.

## Overview

Fishjam supports two livestreaming roles:

- **Streamer**: Broadcasts video and audio to viewers
- **Viewer**: Receives and displays the broadcast

Each livestream room supports one streamer and unlimited viewers. The streamer can send only one video track and one audio track at a time.
...
The livestreaming scenario automatically handles the broadcast setup. You stream normally, and Fishjam distributes your content to all viewers.

:::note
Livestreaming rooms accept only one video track and one audio track. Fishjam ignores additional tracks.
:::
