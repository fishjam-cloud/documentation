...
- `pipEnabled`: A variable deciding whether the Picture-in-Picture is enabled. Defaults to true
- `autoStartPip`: A variable deciding whether the Picture-in-Picture mode should be started automatically after the app is backgrounded. Defaults to false
- `autoStopPip`: A variable deciding whether the Picture-in-Picture mode should be stopped automatically on iOS after the app is foregrounded. Always enabled on Android as PiP is not supported in foreground. Defaults to false
- `pipSize`: A variable deciding the size of the Picture-in-Picture mode. Accepts an object with `width` and `height` properties
...
The `LivestreamView` component is a wrapper around the `WhepClientView` from `react-native-whip-whep`. While you can use props to configure PiP behavior, you can also control it programmatically using the ref:
...
function LivestreamPlayer() {
  const livestreamRef = useRef<LivestreamViewRef>(null);
  const { connect, disconnect }: UseLivestreamResult = useLivestream();

  // TypeScript will provide full type checking and autocompletion
  const handleConnect = async () => {
    try {
      await connect("https://fishjam.io/api/v1/live", "your-token");
    } catch (error) {
