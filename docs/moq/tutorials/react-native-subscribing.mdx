...
It uses [`react-native-moq`](https://github.com/software-mansion/react-native-moq) — React Native bindings for MoQKit, with a small, reactive hooks-based API. For the web equivalent, see [Web Subscribing](./web-subscribing).
...
For audio-only streaming, use [`useAudioPlayer(broadcast)`](https://github.com/software-mansion/react-native-moq/blob/main/docs/API.md#useaudioplayerbroadcast-setup) instead of `useVideoPlayer` — the video track is never subscribed, so no video bandwidth is consumed.
...
It exposes imperative `enterFullscreen()` / `exitFullscreen()` methods on its ref, and the chrome is fully customizable. See [Default UI components](https://github.com/software-mansion/react-native-moq/blob/main/docs/API.md#default-ui-components-react-native-moq-ui) in the API reference.
