...
It uses [`react-native-moq`](https://github.com/software-mansion/react-native-moq) — React Native bindings for MoQKit, with a small, reactive hooks-based API. For the web equivalent, see [Web Publishing](./web-publishing).
...
function PublishScreen() {
  ...
  // Request camera + mic permissions on Android (iOS uses Info.plist)
  useEffect(() => {
  ...
  return (
    <>
      {/* On-screen preview of the camera being captured */}
