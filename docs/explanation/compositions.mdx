...
A scene is not fixed for the life of an output. You can replace it while the composition is running, either straight away or at a chosen point on the composition timeline, so the layout can follow whatever your application is doing. See [Choose inputs and outputs](./../how-to/compositions/inputs-and-outputs) for how to send an update, and the [tutorial](./../tutorials/compositions) to try one.

There are two ways to decide what the layout should be. You can drive it yourself, pushing a new scene whenever something changes, which keeps the layout entirely in your backend's hands. Or you can hand that job to a **template**: a React component that receives the live room state and re-renders itself as participants join, leave, mute, or speak. Templates suit anything where the layout follows the room, since they turn a stream of updates you would otherwise have to send into a component that reacts on its own. See [Write and deploy a template](./../how-to/compositions/write-and-deploy-a-template) to build one.
