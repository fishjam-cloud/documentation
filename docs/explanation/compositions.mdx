...
A composition produces video, it does not serve it. There is nowhere to point a player at a composition, so it always needs an output aimed at something that viewers can actually connect to, such as a [livestream](./livestreams) or an RTMP destination.
