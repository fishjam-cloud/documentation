...
Fishjam uses H.264 by default. To solve an issue with Android emulators, the Sandbox API uses VP8 unless you pass the `videoCodec` query parameter (`vp8` or `h264`). Use H.264 if you want to [compose a room](../how-to/compositions/compose-a-fishjam-room) created with the Sandbox API.

You can override the default codec by setting the `videoCodec` parameter when [creating a room](../api/server/interfaces/RoomConfig#videocodec) using server SDKs.
