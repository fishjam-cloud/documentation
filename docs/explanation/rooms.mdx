...
Fishjam uses H.264 by default, however, to solve an issue with Android emulators, the Sandbox API uses VP8 unless you pass the `videoCodec` query parameter (`vp8` or `h264`). Use H.264 if you want to test [compositions](./compositions) with Sandbox API rooms.
