...
A token is attached to the relay URL as a query parameter (`?jwt=<token>`). The relay validates the token and enforces its scope before allowing any media to flow. Both the Sandbox API and the Server SDK hand you this full **connection URL** (the relay URL with the token already embedded), so the client connects to it directly without assembling anything.
