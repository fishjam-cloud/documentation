...
:::info
This explanation only concerns Fishjam rooms that have type `livestream`.
:::

When livestreaming with Fishjam, you have the ability to chose whether your livestream is `private` or `public`.
...
<Tabs groupId="language">
  ...
  <TabItem value="python" label="Python">
    ...
    private_room = await fishjam_client.create_room(room_type="livestream") # [!code highlight]
    public_room = await fishjam_client.create_room(room_type="livestream", public=True) # [!code highlight]
    ```

  </TabItem>
</Tabs>

The choice you make affects how **viewers** connect to the livestream and has no affect on streamers.
...
<Tabs groupId="language">
  <TabItem value="ts" label="Typescript">
    ...
    const { token } = await fishjamClient.createLivestreamViewerToken(privateRoom.id); // [!code highlight]
    ```
  ...
  <TabItem value="python" label="Python">
    ...
    private_room = await fishjam_client.create_room(room_type="livestream")
    token = await fishjam_client.create_livestream_viewer_token(private_room.id) # [!code highlight]
    ```
