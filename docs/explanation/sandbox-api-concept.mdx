...
The **Sandbox API URL** points to our **room manager**: a hosted HTTP backend you can use to test Fishjam without writing your own server. The room manager creates **rooms** and **peers** for conferencing, and supports **livestreaming** by issuing the streamer and viewer credentials. It is a implementation of that kind of backend, built with our **JavaScript server SDK**.
...
If you expose the room manager in a public deployment, your Sandbox API URL can be leaked (for example through client code or network requests). Anyone who obtains that URL can use your room manager credentials to create rooms and peers on your behalf.
