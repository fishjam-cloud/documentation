...
The **Sandbox API URL** is the room manager endpoint. React/React Native SDK helpers such as `useSandbox({ sandboxApiUrl })` call this URL to create rooms and obtain peer, streamer, or viewer tokens.

The **Fishjam ID** is required by the React/React Native SDK when connecting to the Fishjam media server. After the room manager returns a peer token, pass the Fishjam ID to `FishjamProvider` so the client can connect to your Fishjam media server.
