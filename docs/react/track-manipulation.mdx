...
You can set middleware for your media tracks using `setTrackMiddleware` method. This method accepts a `TrackMiddleware` or `null` for removing previously set middleware.

### Example: Applying a Blur Effect

The following example demonstrates how to apply a custom blur effect to the camera track using `useCamera` hook and middleware.
...
This example provides a button to toggle the blur effect on and off. The `BlurProcessor` handles the actual processing logic (assumed to be implemented elsewhere).
