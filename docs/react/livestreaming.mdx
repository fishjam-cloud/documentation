...
export function useStreamContent(peerToken: string, fishjamUrl: string) {
  const { initializeDevices } = useInitializeDevices();

  const { connect } = useConnection();

  const initializeDevicesAndStream = useCallback(async () => {
    // Initialize devices (camera, microphone, etc.)
    await initializeDevices();

    // Connect to the livestream room
    await connect({
      url: fishjamUrl,
      token: peerToken,
    });
  }, [initializeDevices, connect]);

  useEffect(() => {
    initializeDevicesAndStream();
  }, [initializeDevicesAndStream]);
}
```
