...
- **Conferencing**: For interactive video calls with multiple participants. Uses a split-screen layout showing your local camera (left) and the active speaker (right) with automatic voice activity detection.
- **Livestreaming**: For viewing livestreams. Displays the WHEP stream video in the Picture in Picture window using the native video player's PiP functionality.
