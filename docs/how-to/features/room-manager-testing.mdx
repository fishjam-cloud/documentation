...
# How to Use Room Manager for Testing

**How-to Guide** - _Use Room Manager to test your Fishjam integration without a backend_
...
- `roomType` - Type of room to create (defaults to `full-feature`)

### Room types available:

- `full-feature` - For video/audio conferencing (default)
    ...
    const fetchRoomData = async () => {
      try {
        const response = await fetch(
          `https://fishjam.io/api/v1/connect/${YOUR_APP_UUID}/room-manager?roomName=${roomName}&peerName=${peerName}&roomType=full-feature`,
        );
...
const peerName = `developer_${userIndex}`;
```
...
- [Learn about Room Manager concepts](/explanation/room-manager-concept)
- [Understand security implications](/explanation/security-tokens)

For production deployment:

- [How to set up production server](/how-to/backend/server-setup)
