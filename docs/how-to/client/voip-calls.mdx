...
whose README walks through the native setup in even more detail. To actually run it you
must generate your own APNs and FCM credentials, since VoIP pushes can't be delivered
with ours.
:::
...
(**iOS only**). It registers PushKit at launch so a VoIP push delivered before the JS
bundle loads (cold start) can still ring:
...
- `status` is `connecting`: join [`currentCall`](../../api/mobile/type-aliases/CurrentCall)`.roomName`.
