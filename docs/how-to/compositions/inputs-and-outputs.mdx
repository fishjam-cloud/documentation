...
type: how-to
sidebar_position: 4
description: Choose the right input and output protocols for a composition, from WebRTC to RTMP, HLS, and MP4.
---
...
Point any WHIP publisher at `endpoint_route` under the composition's base URL and authenticate with that token. A `whip_server` input takes WebRTC from anything that speaks WHIP: a phone's camera, a laptop webcam, a browser tab, a screen share, or a hardware encoder. [vdo.ninja](https://vdo.ninja/whip) publishes from a browser, and OBS has WHIP output built in.
...
There is no URL you can point a player at to watch a composition. Both output types **push** the composed video to somewhere else, and viewers connect to that somewhere else:

```text
composition ──push──▶ livestream or RTMP destination ──▶ [viewers]
```

So aim a `whip_client` output at a [livestream](./../../explanation/livestreams) and people watch it through that livestream's WHEP URL. Aim an `rtmp_client` output at a social platform and they watch it there. Without an output, a composition renders for nobody.
