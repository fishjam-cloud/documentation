...
Both types push. A composition is never played back from the Composition API
itself, so viewers connect to whichever destination the output publishes to: a
`whip_client` output pointed at a [livestream](./../../explanation/livestreams)
is watched through that livestream's WHEP endpoint.
