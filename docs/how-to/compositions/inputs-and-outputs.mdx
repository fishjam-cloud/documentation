...
So the destination is what your viewers need, not the composition. A `whip_client` output pointed at a [livestream](./../../explanation/livestreams) is watched through that livestream's WHEP URL, and an `rtmp_client` output pointed at YouTube is watched on YouTube.
