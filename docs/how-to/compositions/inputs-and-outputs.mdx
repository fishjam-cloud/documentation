...
The examples below run against a composition you already have, created as in [Step 1 of the tutorial](./../../tutorials/compositions#step-1-create-a-composition):
...
Point any WHIP publisher at `endpoint_route` under the composition's base URL and authenticate with that token. A `whip_server` input takes WebRTC from anything that speaks WHIP: a phone's camera, a laptop webcam, a browser tab, a screen share, or a hardware encoder. OBS has WHIP output built in.
