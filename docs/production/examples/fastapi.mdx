...
The `Depends` function allows the `FishjamClient` to be provided to the route function as a dependency, which allows for code deduplication and easy mocking.
