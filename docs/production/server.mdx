...
When creating peer, you can also add metadata assigned to that peer. They can be later on read with [mobile SDK](/react-native/metadata)
or [web SDK](/react/metadata). These metadata can be only set when creating peer, and can't be updated later.
