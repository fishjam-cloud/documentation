...
Create a room to get the `roomId` and be able to start adding participants.
...
Create a participant to obtain the participant token allowing your user to join the room.
At any time you can terminate user's access by deleting the participant.
...
Our SDK's come equipped with a Notifier allowing you to subscribe for messages.
