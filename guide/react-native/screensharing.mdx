...
On Android devices, screen share feture is available right after SDK is installed in app. No further actions are required.
...
First, you have to make sure that permissions are granted. To do that, you can call `handleScreencastPermission` method:

```tsx
import { useScreencast } from "@fishjam-cloud/react-native-client";

function Component() {
  const { handleScreencastPermission } = useScreencast();

  useEffect(() => {
    function checkPermissions() {
      const result = await handleScreencastPermission();
      if (result === "granted") {
...
Then, you can enable/disable screen sharing with `toggleScreencast` method. And check current state with `isScreencastOn` property.
...
function ScreenShareButton() {
  ...
  return (
    <Button onPress={toggleScreencast}>
      {isScreencastOn ? "Disable" : "Enable"} screen share
    </Button>
  );
}
```
